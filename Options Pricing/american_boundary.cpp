...
std::vector<double> generate_vector(std::vector<double> x, double epsilon){
    ...
    */
    ...
    double r = gen_uniform(0, epsilon) + x[0];
    ...
    std::vector<double> v = {r, theta_1, theta_2, theta_3, theta_4, theta_5};
...
class American_Option{
    ...
    std::pair<std::vector<double>, double> optimize_exercise_boundary(double a, double b, double epsilon, int m, double T_max){
        
        std::vector<double> x = generate_vector({0,0,0,0,0,0}, epsilon);
        std::vector<double> y;
        ...
        return std::make_pair(x, 1./GRW_price(x,100));
    }
...
int main(){
    ...
    std::pair<std::vector<double>, double> data = O.optimize_exercise_boundary(a, b, epsilon, 10000, T_max);
